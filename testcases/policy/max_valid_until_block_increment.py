...
class MaxValidUntilBlockIncrement(Testing):
    ...
    def __init__(self):
        ...
        self.original_MVUBI = 5760
        self.updated_MVUBI = 5000
        self.min_MVUBI = 1
        self.max_MVUBI = 86400
        self.neo3_only = True
    ...
    def _get_original_max_valid_until_block_increment(self):
        ...
        self.logger.info(f"Get original MVUBI result: {result}")
        # self.check_stack(result['stack'], [('Integer', str(self.original_max_valid_until_block_increment))])

        self.original_MVUBI = int(result['stack'][0]['value'])
        self.updated_MVUBI = self.original_MVUBI + 1000

    def _check_invoke_function_update_max_valid_until_block_increment(self):
        # Step 2: set the max_valid_until_block_increment to updated_MVUBI by rpc invoke_function.
        ...
        self.client.invoke_function(POLICY_CONTRACT_HASH, "setMaxValidUntilBlockIncrement", [
                                    ContractParameter(type="Integer", value=self.updated_MVUBI)])

        # Step 3: get the max_valid_until_block_increment again, it should be the default MVUBI.
        result = self.client.invoke_function(POLICY_CONTRACT_HASH, "getMaxValidUntilBlockIncrement", [])
        self.check_stack(result['stack'], [('Integer', str(self.original_MVUBI))])
    ...
    def _check_no_permission_update_max_valid_until_block_increment(self):
        ...
        self.logger.info(f"No permission update MVUBI transaction sent: {tx_id}")
        ...
        self.logger.info(f"No permission update MVUBI application log: {application_log}")
    ...
    def _check_committee_update_max_valid_until_block_increment(self, mvubi: int):
        # Step 9: set the max_valid_until_block_increment to `max_valid_until_block_increment` by validators
        tx = self._make_update_max_valid_until_block_increment_tx(mvubi)
        tx_id = self.client.send_raw_tx(tx.to_array())['hash']
        self.logger.info(f"committee update MVUBI transaction sent: {tx_id}")
        ...
        application_log = self.client.get_application_log(tx_id)
        self.logger.info(f"Committee update MVUBI application log: {application_log}")
        assert 'txid' in application_log and tx_id == application_log['txid']
        assert 'executions' in application_log and len(application_log['executions']) == 1
        ...
        result = self.client.invoke_function(POLICY_CONTRACT_HASH, "getMaxValidUntilBlockIncrement", [])
        self.logger.info(f"Get updated MVUBI result: {result}")
        self.check_stack(result['stack'], [('Integer', str(mvubi))])

    def _check_max_valid_until_block_increment_range(self):
        # Step 13: check the max_valid_until_block_increment range
        tx = self._make_update_max_valid_until_block_increment_tx(self.min_MVUBI - 1)
        tx_id = self.client.send_raw_tx(tx.to_array())['hash']
        self.logger.info(f"update MVUBI to {self.min_MVUBI - 1} transaction sent: {tx_id}")
        ...
        application_log = self.client.get_application_log(tx_id)
        self.logger.info(f"update MVUBI to {self.min_MVUBI - 1} application log: {application_log}")
        assert 'txid' in application_log and tx_id == application_log['txid']
        assert 'executions' in application_log and len(application_log['executions']) == 1

        # Check the execution, the max_valid_until_block_increment is {self.min_MVUBI - 1}, so the transaction is fault.
        execution = application_log['executions'][0]
        self.check_execution_result(execution, exception=f'MVUBI must be between [{self.min_MVUBI}, {self.max_MVUBI}]')

        tx = self._make_update_max_valid_until_block_increment_tx(self.max_MVUBI + 1)
        tx_id = self.client.send_raw_tx(tx.to_array())['hash']
        self.logger.info(f"update MVUBI to {self.max_MVUBI + 1} transaction sent: {tx_id}")
        ...
        application_log = self.client.get_application_log(tx_id)
        self.logger.info(f"update MVUBI to {self.max_MVUBI + 1} application log: {application_log}")
        assert 'txid' in application_log and tx_id == application_log['txid']
        assert 'executions' in application_log and len(application_log['executions']) == 1

        # Check the execution, the max_valid_until_block_increment is {self.max_MVUBI + 1}, so the transaction is fault.
        execution = application_log['executions'][0]
        self.check_execution_result(execution, exception=f'MVUBI must be between [{self.min_MVUBI}, {self.max_MVUBI}]')
    ...
    def run_test(self):
        ...
        self._check_committee_update_max_valid_until_block_increment(self.updated_MVUBI)

    # Post test: set the max_valid_until_block_increment to original MVUBI.
    def post_test(self):
        self._check_committee_update_max_valid_until_block_increment(self.original_MVUBI)
